...
function TrackerApp() {
  ...
  const [roadmapEditingProblem, setRoadmapEditingProblem] = useState(null)
  ...
  function renderDashboard() {
    ...
    return <>
      ...
      <div className="mb-9 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <input type="text" value={dashboardGreeting} onChange={(event) => setDashboardGreeting(event.target.value)} onBlur={async () => { try { const updated = await api.updateProfile({ welcome_message: dashboardGreeting }); setDashboardGreeting(updated.welcome_message || 'Welcome back') } catch (error) { setCoreError(error.message) } }} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur() }} aria-label="Dashboard greeting" className="w-full max-w-3xl bg-transparent p-0 text-3xl font-semibold leading-tight tracking-tight text-slate-950 outline-none placeholder:text-slate-400 dark:text-white dark:placeholder:text-slate-500 sm:text-[2.2rem]" placeholder="Welcome back" />
  ...
  function renderRoadmap() {
    ...
    return <>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#303030]">
              {matchingProblems.map((problem, index) => {
                ...
                return <tr key={problem.id} className="hover:bg-slate-50 dark:hover:bg-[#242424]/40">
                  ...
                  <td className="px-5 py-4 text-center">{problem.status === 'solved' ? <span className="font-semibold text-emerald-600 dark:text-emerald-400">Solved</span> : <span className="text-slate-400">—</span>}</td><td className="px-5 py-4"><div className="flex flex-wrap gap-1.5">{problem.leetcode_url && <PracticeLink href={problem.leetcode_url} label="LeetCode" tone="leetcode" />}{problem.gfg_url && <PracticeLink href={problem.gfg_url} label="GFG" tone="gfg" />}{problem.article_url && <PracticeLink href={problem.article_url} label="Article" tone="article" />}{problem.youtube_url && <PracticeLink href={problem.youtube_url} label="YouTube" tone="youtube" />}{user?.role === 'admin' ? <button type="button" title="Edit practice links" aria-label={`Edit practice links for ${problem.title}`} onClick={() => setRoadmapEditingProblem(problem)} className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-slate-300 bg-white text-slate-500 hover:bg-slate-50 hover:text-slate-800 dark:border-[#3a3a3a] dark:bg-[#171717] dark:text-slate-400 dark:hover:bg-[#242424] dark:hover:text-slate-100"><EditIcon /></button> : null}</div></td>
                </tr>
              })}
            </tbody>
          </table>
      ...
      {roadmapEditingProblem && <EditLinksModal problem={roadmapEditingProblem} onClose={() => setRoadmapEditingProblem(null)} onSave={async (fields) => { await handleProblemUpdate(roadmapEditingProblem.id, fields); setRoadmapEditingProblem(null) }} />}
    </>
  }
