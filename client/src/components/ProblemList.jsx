...
function PatternPopover({ problem, onClose }) {
  return (
    <div className="absolute right-0 top-11 z-20 w-80 max-w-[calc(100vw-2rem)] rounded-xl border border-slate-200 bg-white p-4 text-left shadow-xl dark:border-slate-700 dark:bg-slate-900" role="dialog" aria-label={`Pattern details for ${problem.title}`}>
      ...
      {(problem.time_complexity || problem.space_complexity) && <div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800"><span className="block text-slate-400">Time</span><span className="mt-0.5 block font-medium text-slate-700 dark:text-slate-200">{problem.time_complexity || '—'}</span></div><div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800"><span className="block text-slate-400">Space</span><span className="mt-0.5 block font-medium text-slate-700 dark:text-slate-200">{problem.space_complexity || '—'}</span></div></div>}
...
function EditLinksModal({ problem, onClose, onSave }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><div role="dialog" aria-modal="true" aria-labelledby="edit-links-title" className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"><div className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800"><div><p className="text-xs font-semibold uppercase tracking-wide text-violet-600 dark:text-violet-400">Edit resources</p><h2 id="edit-links-title" className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{problem.title}</h2></div><button type="button" title="Close editor" aria-label="Close editor" onClick={onClose} className="text-2xl leading-none text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">×</button></div><form onSubmit={submit}><div className="space-y-4 p-5">{fields.map(([key, label, placeholder]) => <label key={key} className="block"><span className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">{label}</span><input value={values[key]} onChange={(event) => update(key, event.target.value)} placeholder={placeholder} type="url" inputMode="url" className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100" /></label>)}{error && <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{error}</p>}</div><div className="flex justify-end gap-2 border-t border-slate-200 px-5 py-4 dark:border-slate-800"><button type="button" onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</button><button type="submit" disabled={saving} className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white hover:bg-violet-700 disabled:cursor-wait disabled:opacity-60">{saving ? 'Saving…' : 'Save changes'}</button></div></form></div></div>
}
...
function ProblemList({ problems, onStatusChange, onBookmarkChange, onRevisionChange, onProblemUpdate }) {
  ...
  return <>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {problems.map((problem, index) => <tr key={problem.id} className="align-middle hover:bg-slate-50 dark:hover:bg-slate-800/40">
    ...
    {!problems.length && <div className="mt-3 rounded-xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">No problems match this view.</div>}
