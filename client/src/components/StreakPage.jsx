...
function StreakPage() {
  ...
  return <>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6" aria-label="Yearly activity heatmap">
      <div className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
          ...
          <select aria-label="Activity year" value={selectedYear} onChange={(event) => setSelectedYear(Number(event.target.value))} className="cursor-pointer rounded-xl border border-slate-200 bg-slate-100 px-4 py-2.5 font-medium text-slate-700 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
        ...
        <div className="flex min-w-max items-start gap-4">
          {months.map((month) => <div key={`${selectedYear}-${month.label}`} className="shrink-0">
            ...
            <div className="grid grid-flow-col grid-rows-7 auto-cols-[14px] gap-1" aria-label={`${month.label} ${selectedYear}`}>
              {month.cells.map((day, index) => day ? <div key={day.date} title={`${day.label}: ${day.count} ${day.count === 1 ? 'problem' : 'problems'} solved`} aria-label={`${day.label}: ${day.count} ${day.count === 1 ? 'problem' : 'problems'} solved`} className={`h-3.5 w-3.5 rounded-[3px] border border-slate-200 dark:border-slate-700 ${day.future ? 'opacity-35' : ''} ${levelClasses[level(day.count)]}`} /> : <div key={`empty-${selectedYear}-${month.label}-${index}`} className="h-3.5 w-3.5" aria-hidden="true" />)}
            </div>
          </div>)}
        </div>
      </div>

      <div className="mt-5 flex items-center justify-end gap-2 text-xs text-slate-400 dark:text-slate-500"><span>Less</span>{levelClasses.map((className, index) => <span key={index} className={`h-3.5 w-3.5 rounded-[3px] border border-slate-200 dark:border-slate-700 ${className}`} />)}<span>More</span></div>
    </section>
  </>
}
